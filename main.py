...
for search in readSearches():
    ...
    for pageNumber in range(1, pageCount + 1, 1):
        ...
        page = requests.get(str(firstPage) + str('&currentPage=') +
                            str(pageNumber) + '&postcode=' + str(search[5]))
        ...
        for article in soup.select('article.row.search-result'):
            ...
            descriptions1.append(article.find('span',
                                              class_='mp-listing-description').text)
            try:
                descriptions2.append(re.sub('\s', ' ', article.find('span',
                                                                    class_='mp-listing-description-extended wrapped').text).strip())
            except:
                descriptions2.append('')
                pass
            prices.append((article.find('span', class_='price-new').text)
                          .strip())
            cities.append(article.find('div', class_='location-name').text)
            links.append(article.get('data-url'))

        descriptions = [description[0] if description[1] is None
                        else ''.join((description[0], description[1]))
                        for description in
                        zip_longest(descriptions1, descriptions2)]
        dates = [datetime.date.today().strftime("%d %b. '%y")
                 if date == 'Vandaag'
                 ...
                 if date == 'Gisteren'
                       ...
                       datetime.timedelta(1)).strftime("%d %b. '%y")
                 if date == 'Eergisteren' else date for date in dates]
        prices = [price
                  if (price == 'Bieden' or
                      price == 'Gereserveerd' or
                      price == 'Zie omschrijving' or
                      price == 'Gratis' or
                      price == 'N.o.t.k.' or
                      price == 'Ruilen' or
                      price == 'Op aanvraag')
                  else float(price[2:].replace('.', '').replace(',', '.'))
                  for price in prices]

        for i in range(len(dates)):
            siteArticles.append((titles[i], dates[i], descriptions[i],
                                 prices[i], cities[i], links[i]))
        ...
        if tuple((siteArticle[0], siteArticle[1])) not in dbList:
            if (
                ...
                if type(siteArticle[3]) == float:
                    if (siteArticle[3] <= int(search[1]) and
                            siteArticle[3] > int(search[2])):
                        articleList.append({"title": siteArticle[0],
                                            "date": siteArticle[1],
                                            "description": siteArticle[2],
                                            "price": siteArticle[3],
                                            "city": siteArticle[4],
                                            "link": siteArticle[5]})
                        # sendMail(title=siteArticle[0], price=siteArticle[3],
                        # description=siteArticle[2], city=siteArticle[4],
                        # link=siteArticle[5], date=siteArticle[1])
                        print('float send\n')

                    else:
                        pass
                else:
                    if (siteArticle[3] == 'Gratis' or
                            siteArticle[3] == 'Gereserveerd' or
                            siteArticle[3] == 'Ruilen'):
                        pass
                        ...
                        if siteArticle[3] < int(search[3]):
                            articleList.append({"title": siteArticle[0],
                                                ...
                                                "city": siteArticle[4],
                                                "link": siteArticle[5]})
                            # sendMail(title=siteArticle[0],
                            # price=(str(siteArticle[3]) +' (Bieden)'),
                            # description=siteArticle[2], city=siteArticle[4],
                            # link=siteArticle[5], date=siteArticle[1])
                            print('Bieden send\n')
                            pass
