...
for search in readSearches():
    ...
    cookie.click()
    time.sleep(2)
    categories = driver.find_elements_by_xpath("//div[@id='page-wrapper']/div[@class='l-page']/section[@id='left-column-container']/div[@id='search-attributes']/form[@id='search-attributes-form']/div[@class='relevant-categories filter-section']/ul/li[@class='level-two parent-category-id-820']/a/span[@class='category-name']")
    for category in categories:
        try:
            if category.get_attribute('innerHTML') == 'Mobiele telefoons | Apple iPhone':
                category.click()
        except:
            pass
    if db[search] == None:
